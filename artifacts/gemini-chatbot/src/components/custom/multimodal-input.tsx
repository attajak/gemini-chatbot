...
const suggestedActions = [
  { title: "Help me book a flight", label: "from San Francisco to London", action: "Help me book a flight from San Francisco to London" },
  { title: "What is the status", label: "of flight BA142 flying tmrw?", action: "What is the status of flight BA142 flying tmrw?" },
];

export function MultimodalInput({
  input, setInput, isLoading, stop, attachments, setAttachments, messages, append, handleSubmit,
}: {
  input: string;
  ...
  append: (message: Message | CreateMessage, opts?: ChatRequestOptions) => Promise<string | null | undefined>;
  handleSubmit: (event?: { preventDefault?: () => void }, opts?: ChatRequestOptions) => void;
}) {
  ...
  const handleFileChange = useCallback(async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setAttachments((cur) => [...cur, ...(uploaded.filter(Boolean) as Attachment[])]);
    } catch (err) {
